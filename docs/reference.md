...
| Drawings | ✏️ `check_for_unattached_dimensions` | Flag dimensions not attached to part geometry in drawings selected in the Document Manager (or by `marks`) and cloud them. A point is attached when it coincides with a part corner or edge midpoint, a bolt/hole centre or axis, or a grid/centre line. Views with part shortening are handled. None of the target drawings can be the active drawing, and all must be up to date. Magenta clouds are drawn at every unattached point | `marks` (list[str], optional) |
