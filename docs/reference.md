...
| Drawings | ✏️ `detect_collisions_between_marks` | Detect collisions between part marks in the active drawing's views and insert magenta revision clouds around each colliding pair | `view_keys` (list[str], optional - processes all views when omitted) |
