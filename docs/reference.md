...
| Operations | `check_for_orphans` | Check for orphaned embeds or rebars within bounding box of selected parts/assemblies. Returns orphaned elements. Mode "embeds" checks for embedded details not attached to selected elements. Mode "rebars" checks for reinforcement not attached to selected elements. If `attach` is true, orphaned elements are attached to their parent assembly. | `mode` ("embeds" or "rebars", default: "embeds"), `attach` (boolean, default: false) |
| Operations | `check_for_invalid_objects` | Find invalid objects (missing profile/material, invalid geometry, no father) in selected objects and their bounding boxes. Returns parts, reinforcements, and assemblies with validation issues | - |
