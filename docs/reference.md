...
| Drawings | ⚠️ `arrange_colliding_drawing_marks` | Detect colliding part marks in the active drawing's views, automatically rearrange `Mark`/`MarkSet` objects via the `TeklaMCPArrangeMarks` macro and insert magenta revision clouds around pairs still colliding afterwards (weld marks are not moved) | `view_keys` (list[str], optional - processes all views when omitted) |
...
| Drawings | ⚠️ `delete_view_clouds` | Delete all clouds from model views. Clouds placed outside any model view are not affected | `view_keys` (list[str], optional - processes all views when omitted) |
