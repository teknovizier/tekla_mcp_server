...
| Drawings | 🔒 `check_for_unmarked_objects` | Find parts, reinforcement and bolts with no mark pointing at them. An embedded detail subassembly is one `parts` unit - a mark on any one of its member parts covers the whole assembly. Reinforcement and bolts inside the assembly are still checked individually, not collapsed | `view_keys` (list[str], optional - checks every non-sheet view in the active drawing when omitted), `categories` (list[str]: `all`/`parts`/`reinforcement`/`bolts`, default: `all`), `limit` (int, default: `200`) |
