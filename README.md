...
| Properties | `get_elements_properties` | Retrieve structured data about selected elements (assemblies or parts) including key properties (position, GUID, name, profile, material, finish, Tekla class), UDAs, and any defined report properties | `report_props_definitions` |
| Properties | `set_elements_properties` | Set properties (name, profile, material, Tekla class, finish) and user-defined attributes (UDAs) on selected elements | `name`, `profile`, `material`, `tekla_class`, `finish`, `user_properties` |
...
- Model attributes directory (`{ModelPath}/attributes`)
